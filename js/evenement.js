        ...
        success: function(response) {
            ...
            response.forEach(elt => {
                ...
                $('#tbody').append("\
                    <tr>\
                    <td>" + elt['intitule'] + "</td>\
                    <td>" + ("0" + dates.getDate()).slice(-2) + "/" + ("0" + (dates.getMonth() + 1)).slice(-2) + "/" + dates.getFullYear() + " au " + ("0" + datef.getDate()).slice(-2) + "/" + ("0" + (datef.getMonth() + 1)).slice(-2) + "/" + datef.getFullYear() + "</td>\
                    <td>" + elt['heure_r'] + "</td>\
                    <td>" + elt['heure_f'] + "</td>\
                    <td>" + elt['lieux'] + "</td>\
                    <td>\
                    <a href='#' onclick='goToEvent(" + elt['id'] + ",\"" + elt['evenement'] + "\")' class='nav-link'>\
...
function goToEvent(id) {
    $.ajax({
        type: 'GET',
        url: baseUrl + "/viewset/gestion/evenement/" + id,
        success: function(response) {
            clearForm()
